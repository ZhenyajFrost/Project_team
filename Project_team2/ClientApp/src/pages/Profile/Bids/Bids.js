...
function Bids() {
    ...
    useEffect(() => {
        setPagination(bidsDisplay === 'list' && width <= 381 ? { page: 1, pageSize: 4 } : { page: 1, pageSize: 8 })
    }, [bidsDisplay])
    ...
    return (
        <div className={css.container} style={{ padding: '0' }}>
            ...
            <div className={css.display}>
                <Button className={css.hr} onClick={handleButtonSearch}>Search</Button>
            </div>


            {totalCount === 0 ? "No lots Found" : isLoading ? <Loader /> : <BidsContainer bids={bids} display='list' bidStyle={bidStyle} />}
